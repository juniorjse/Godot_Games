...
func attack() -> void:
	var condition: bool = not attacking and not crouching and not defending
	if Input.is_action_just_pressed("attack") and is_on_floor():
		attacking = true
		player_sprite.normal_attack = true
		$attack.play() #ataque_canhão

	elif Input.is_action_just_pressed("magic_attack") and is_on_floor() and stats.current_mana >= magic_attack_cost:
		attacking = true
...
func vertical_movement_env() -> void:
	...
	if Input.is_action_just_pressed("ui_select") and jump_count < 2 and can_track_input and not attacking:
		...
		jump_count += 1
		spawn_effect("res://scenes/particles/dust/jump_effect.tscn", Vector2(0, 5), flipped)
